...
data "aws_ecr_repository" "demo-app" {
...
module "container_definition" {
  ...
  container_image = "${data.aws_ecr_repository.demo-app.repository_url}:latest"
...
resource "aws_ecs_cluster" "demo-app" {
...
data "aws_iam_policy_document" "task_role" {
  statement {
    ...
    resources = ["${aws_ecs_cluster.demo-app.arn}"]
  }
}
...
resource "aws_ecs_service" "demo-app" {
  ...
  task_definition = "${aws_ecs_task_definition.demo-app.id}"
  cluster         = "${aws_ecs_cluster.demo-app.arn}"
  ...
  depends_on = ["aws_lb_listener.demo-app"]
}
